...
display(HTML(gf.makeButtons(sites, "plot_***_AboveWind.jpg", "SourncePresent")))
```

### Are sounds of interest within typical conditions?
```{python}
#| echo: false
display(HTML(standardTextDict["sources of interest typical"]))
display(HTML(gf.makeButtons(sites, "plot_***_status.jpg", "FOI")))
```
:::
...
### IHow much noise from large vessels may be interfering with whale communication?
```{python}
#| echo: false
...
```
#### How does the level of traffic impact change conditions?
```{python}
#| echo: false
