...
# Wrap the button group in a container div with a unique ID
display(HTML(f'''
...
# Add color-changing script for this group only
colorScript = """
<script>
document.addEventListener("DOMContentLoaded", function() {
    ...
    buttons.forEach(button => {
        button.addEventListener('click', () => {
            buttons.forEach(btn => btn.style.backgroundColor = '#FFFFFF');  // reset all
            button.style.backgroundColor = '#ADD8E6';  // highlight clicked
        });
    });

    // Highlight the first button by default
    if (buttons.length > 0) {
        buttons[0].style.backgroundColor = '#ADD8E6';
    }
});
</script>
"""
display(HTML(colorScript))
...
display(HTML(f'''
<div id="button-group-YearSPL">
    {gf.makeButtons(sites, "plot_***_YearSPL.jpg", "YearSPL")}
</div>
'''))
...
<script>
document.addEventListener("DOMContentLoaded", function() {
    const container = document.querySelector('#button-group-YearSPL');
    if (!container) return;
...
display(HTML(gf.makeButtons(sites, "plot_***-200-1000_status.jpg", "FOI")))
```
:::
